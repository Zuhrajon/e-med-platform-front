...
export default function ReceptionProfilePage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setIsEditing(false)
      setSuccess('Профиль сотрудника обновлён.')
    } catch (saveError) {
  ...
  return (
    <div className="w-full px-6 py-10">
      <AdminPageHeader
        title="Профиль"
        description="Личные данные сотрудника ресепшена и управление паролем."
      />
      ...
      <div className="mt-8 max-w-4xl space-y-6">
        <ProfileDetailsSection
          title="Личные данные"
          subtitle="Изменения применяются к серверному профилю сотрудника."
          form={form}
          isEditing={isEditing}
          isSaving={isSaving}
          readOnlyBadges={[{ label: 'Роль', value: 'Ресепшен' }]}

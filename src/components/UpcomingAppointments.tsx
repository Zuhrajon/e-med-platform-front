  ...
  return (
    <section className="w-full">
      <div className="mb-8 flex items-center justify-between">
        ...
        {showAllLink && (
          <button className="text-[16px] font-semibold text-slate-900">
            Все записи
          </button>
        )}
      </div>

      <div className={variant === 'compact' ? 'flex flex-col gap-4' : 'grid grid-cols-1 gap-6 md:grid-cols-2'}>
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-4">
                ...
                <span
                  className={`rounded-full px-5 py-2 text-[16px] font-medium ${getStatusStyles(
                    appointment.status
                  )}`}
                >
                  {appointment.status}
              ...
              {variant === 'detailed' && (
                <>
                  ...
                  <div className="flex gap-3">
                    ...
                    <button className="flex-1 rounded-xl bg-sky-600 px-4 py-3 text-[16px] font-semibold text-white transition hover:bg-sky-700">
                      Перенести
                    </button>
                  </div>
                </>
